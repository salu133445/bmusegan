...
from bmusegan.components import Discriminator, Generator, Refiner
from bmusegan.components import End2EndGenerator
from bmusegan.metrics import Metrics
from bmusegan import utils
...
class Model(object):
    ...
    def get_statistics(self):
        ...
        num_par = utils.get_num_parameter(tf.trainable_variables(
            self.scope.name))
        num_par_g = utils.get_num_parameter(self.G.vars)
        num_par_d = utils.get_num_parameter(self.D_fake.vars)
    ...
    def save_samples(self, filename, samples, save_midi=False, shape=None):
        ...
        imagepath = os.path.join(self.config['sample_dir'],
                                 '{}.png'.format(filename))
        utils.save_image(imagepath, samples, shape)
        if save_midi:
            midipath = os.path.join(self.config['sample_dir'],
                                    '{}.mid'.format(filename))
            utils.save_midi(midipath, samples, self.config)

    def run_sampler(self, targets, feed_dict, save_midi=False):
        ...
        filename = str(tf.train.global_step(self.sess, self.global_step))
...
class GAN(Model):
    ...
    def train(self, train_config):
        ...
        # Load data
        print('[*] Loading data...')
        x_train = sa.attach(train_config['train_x'])
        x_train = x_train[..., self.config['track_filter']]
        x_train = x_train.reshape(
            -1, self.config['num_bar'], self.config['num_timestep'],
            self.config['num_pitch'], self.config['num_track'])
        ...
        for epoch in range(train_config['num_epoch']):
            ...
            for batch in range(num_batch):
                ...
                for _ in range(num_critics):
                    self.sess.run(self.d_step, feed_dict_batch)
                self.sess.run(self.g_step, feed_dict_batch)

                time_batch = time.time() - batch_start_time

                d_loss_, g_loss_ = self.sess.run([self.d_loss, self.g_loss],
                                                 feed_dict_batch)
                ...
                log_batch.write("{:2d}, {:4d}, {:6.2f}, {:14.6f}, {:14.6f}\n"
                                "".format(epoch, batch, time_batch, -d_loss_,
                                          g_loss_))

                # run sampler
                if (counter%100) == 0 or (counter < 300 and counter%20 == 0):
                    self.run_sampler(self.G.tensor_out, feed_dict_sample, False)
            ...
            if not train_config['verbose']:
                ...
                print("  {:2d}  | {:8.2f} | {:14.6f} | {:14.6f}".format(
                    epoch, time_epoch, -d_loss_, g_loss_))

            log_epoch.write("{:2d}, {:6.2f}, {:14.6f}, {:14.6f}\n".format(
                epoch, time_epoch, -d_loss_, g_loss_))
    ...
    def run_eval(self, target, feed_dict):
        """Run evaluation."""
        result = self.sess.run(target, feed_dict)
        filename = str(tf.train.global_step(self.sess, self.global_step))
        reshaped = (result > 0.5).reshape((-1,) + result.shape[2:])
...
class RefineGAN(Model):
    ...
    def run_eval(self, target, feed_dict):
        ...
        result = self.sess.run(target, feed_dict)
        filename = str(tf.train.global_step(self.sess, self.global_step))
        reshaped = result.astype(bool).reshape((-1,) + result.shape[2:])
        mat_path = os.path.join(self.config['eval_dir'], filename+'.npy')
        _ = self.metrics.eval(reshaped, mat_path=mat_path)
